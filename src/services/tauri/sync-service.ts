...
class SyncService {
  ...
  async testConnection(config: SyncRepoConfig): Promise<boolean> {
    return invoke<boolean>('sync_test_connection', { config });
  }
  ...
  /** 浏览远程目录 */
  async browseRemote(configId: string, path?: string): Promise<RemoteEntry[]> {
    return invoke<RemoteEntry[]>('sync_browse_remote', { configId, path });
  }
   ...
   * @param excludePatterns 可选，调用方实时的黑名单（不传则后端使用清单中的值）
   */
  async diff(
    localPath: string,
