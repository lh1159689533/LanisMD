...
LanisMD 采用 Tauri 2.x 的 **Rust 后端 + WebView 前端** 混合架构。前端通过 React 构建 UI 交互层，后端通过 Rust 处理系统级操作（文件系统、窗口管理、原生 API）。
...
Tauri 2.x 提供三种通信方式，LanisMD 根据场景分别选用：
...
1. **极简 API，低样板代码**：Zustand 的 `create()` API 只需几行代码即可创建 Store，无需像 Redux 那样定义 action、reducer、dispatch。对于 LanisMD 的中等复杂度状态，Zustand 足够
...
| 平台      | 路径                                                          |
| ------- | ----------------------------------------------------------- |
| macOS   | `~/Library/Application Support/com.lanismd.app/config.json` |
| Windows | `%APPDATA%\com.lanismd.app\config.json`                     |
| Linux   | `~/.config/com.lanismd.app/config.json`                     |
...
Milkdown 采用声明式插件系统，所有插件通过 `Milkdown.use()` 注册。LanisMD 按功能模块组织插件：
...
{
  "identifier": "default",
  "description": "Default capabilities for LanisMD",
  "windows": ["main"],
  "permissions": [
    "core:default",
    "fs:default",
